...
export function buildPrReviewOrchestratorPrompt(
  ...
  const reviewInstruction = [
    ...
    `In that resumed parent turn, call \`${PR_REVIEW_ORCHESTRATOR_TOOLS.readChildResult}\` once for each \`child_chat_id\`. Read both complete results and verify their non-empty hexadecimal \`headRefOid\` values identify the same commit: normally both are the same full 40-character SHA; if exactly one is a 7-39 character prefix, accept it only when it exactly prefixes the other full SHA, then use the full SHA. If either result failed, omitted its SHA, or identifies a different commit, stop without publishing and report the problem. Otherwise reconcile and deduplicate them into one evidence-backed finding set. Keep the highest defensible severity when findings overlap; discard findings that are unsupported by the changed code; mention material reviewer disagreement in the final conversation summary.`,
  ].join("\n");
