...
export function makeDesktopContentSecurityPolicy(input: DesktopProtocolRegistrationInput): string {
  ...
  // the build-configured OTLP endpoints. Those environment origins are not known
  // when this response policy is created, so restrict connections by the network
  // schemes the client supports instead of by host.
