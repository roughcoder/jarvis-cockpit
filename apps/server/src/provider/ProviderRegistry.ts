/**
 * ProviderRegistry aggregates per-instance snapshot streams into a
...
export const make = Effect.gen(function* () {
  ...
  const liveSubsRef = yield* Ref.make<ReadonlyMap<ProviderInstanceId, ProviderInstance>>(new Map());
  ...
  const refresh = Effect.fn("refresh")(function* (provider?: ProviderDriverKind) {
    ...
    const sources = yield* getLiveSources;
    const providerSource = sources.find((candidate) => candidate.instanceId === defaultInstanceId);
    if (!providerSource) {
  ...
  const refreshInstance = Effect.fn("refreshInstance")(function* (instanceId: ProviderInstanceId) {
    const sources = yield* getLiveSources;
