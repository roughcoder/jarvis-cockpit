 ...
 * without trying to spawn real `codex` / `claude` binaries. That keeps the
 * assertions focused on registry routing
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  // All drivers need `NodeServices` (ChildProcessSpawner + FileSystem + Path).
  const testLayer = ServerConfig.layerTest(process.cwd(), {
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      const textGenerations = [codex!.textGeneration, claude!.textGeneration];
      expect(new Set(textGenerations).size).toBe(textGenerations.length);
      const snapshots = [codex!.snapshot, claude!.snapshot];
